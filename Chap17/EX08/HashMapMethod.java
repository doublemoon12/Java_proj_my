...
public class HashMapMethod {
	...
	public static void main(String[] args) {
		...
		hMap1.put(2, "나다라");
		hMap1.put(1, "가나다");
		hMap1.put(3, "다라마");	//키는 중복되서 저장 될 수 없다. 값을 넣을 경우 기존의 값이 수정된다.
		//hMap1.put(3, "가가가");
		...
		hMap2.replace(1, "가가가");
		hMap2.replace(4, "라라라");	//key4는 존재하지 않는다. 작동이 안됨(예외도 아니라 발생안됨)
		System.out.println(hMap2);
		
		//4. replace(K key, V oldValue, V newValue)	: oldValue를 newValue로 수정
		hMap2.replace(1, "가가가", "나나나");
		hMap2.replace(2, "다다다", "나나나");	//key와 value가 정확히 일치해야 수정됨.
		
		System.out.println(hMap2);
		
		//5. get(Object key)	: Map의 값을 가져올때. Key를 넣어줘야 Key에 할당된 Value를 출력.
		System.out.println(hMap2.get(1));	//주의 1은 index가 아니라 key
		...
		Set<Integer> keySet = hMap2.keySet();	//hMap2에 저장된 keySet을 추출
