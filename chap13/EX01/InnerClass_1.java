...
//이너클래스(inner Class) 	: 클래스 내부의 클래스
	//1. 인스턴스 이너클래스 	: 클래스 이름앞에 static이 없는 내부클래스
		//외부 객체를 먼저 생성 후 자신의 객채 생성 후 사용가능
		//A a = new A();	<=== 외부 객체
		//A.B b = a.new A();	<=== 내부 객체
	//2. Static 이너클래스 	: static이 붙어있는 내부클래스
		//
	//3. 지역 이너클래스 	: 클래스의 인스턴스 메소드 내부에 있는 클래스

	//특징 :	1. Outer Class의 접근제어자 제한 없이 접근이 가능
		// 	2. 다른 객체를 생성하지 않고, 객체의 필드나 메소드 접근을 빠르게 가능
	
	//클래스 파일	: A.class, A$B.class	<==byte코드
...
class A{		//Outer Class	: 모든 접근 제어자를 내부 클래스에서 접근 가능
	...
	private int d = 6;		//다른 패키지에서 접근 불가능, 같은 패키지에서 접근 불가능, 같은 클래스에서 접근 가능
...
public class InnerClass_1 {
	...
	public static void main(String[] args) {
		...
		A.B b = a.new B();	//A.B 	: 외부클래스.내부클래스 b : 내부클래스 인스턴스 변수
							//a.new B();	<=== 외부 인스턴스명.new 내부클래스생성자 호출
		//3. 내부 클래스의 메소드 호출
		b.bcd();
